...
def main():
    global sheetArray
    global kanjiSheetArray
    global kanjiDict
    global wordSheetArray
    ...
    wordSheetArray = wordSheet.to_array()
    ...
    kanjiSheetArray = kanjiSheet.to_array()
    ...
    if filechoice == 'create': #make a new one
        outputFileName = raw_input('Enter the path of the new spreadsheet you want to create: ')
        sheet = createNewSheet(outputFileName)
    else:                      #load existing
        outputFileName = filechoice
        sheet = pyexcel.get_sheet(file_name=outputFileName)
        ...
        if len(line) == 1 or line[1] == "":
            ...
            for vocabLine in wordSheetArray:
                if currentKanji in vocabLine[0]:
                    ...
                    T.insert(INSERT, replaceKanji(
                                         currentKanji, 
                                         kanjiDict[currentKanji][0] + kanjiDict[currentKanji][1], 
                                         vocabLine[0], 
                                         vocabLine[1]))
                    T.pack(side=LEFT)
                    textArray.append(T)
            ...
            newLineButton = Tkinter.Button(
                ButtonsFrame,
                text="Add blank line",
                command=lambda:addNewLine(entriesFrame, frameArray, checkboxArray, textArray))
            ...
            B = Tkinter.Button(
                ButtonsFrame, 
                text="Add to sheet", 
                command=lambda:print_close(checkboxArray, textArray, rowNum, top))
            B.pack(side=RIGHT)
...
def addNewLine(entriesFrame, frameArray, checkboxArray, textArray):
    frame = Frame(entriesFrame)
    frame.pack()
    frameArray.append(frame)
    checkvar = IntVar()
...
def makeKanjiDict():
    global kanjiSheetArray
    global kanjiDict
    for entry in kanjiSheetArray:
        kanjiDict[entry[0].strip()] = entry[1:]
...
def replaceKanji(kanji2Replace, kanjiReadings, wordKanji, wordReading):
...
def print_close(checkArray, textArray, rowNum, top):
    global sheetArray
    ...
    for i in range(0, len(checkArray)):
        if checkArray[i].get() == 1:
            entryString += textArray[i].get('1.0', 'end-1c') + "\n"
    
    #append if there is only one column in the row, or overwrite if there is more than one
    if len(sheetArray[rowNum]) == 1:
        sheetArray[rowNum].append(entryString.rstrip())
    else:
        sheetArray[rowNum][1] = entryString.rstrip()

    #write to file
    pyexcel.Sheet(sheetArray).save_as(outputFileName)
...
def createNewSheet(name):
    global kanjiSheetArray
    newSheetArray = []
    for line in kanjiSheetArray:
